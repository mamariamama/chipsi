...
# ... (весь предыдущий код базового класса ArtAgent точно такой же)
# Приводим только специфичную часть класса VangoghAgent

class VangoghAgent(ArtAgent):
    """Агент в стиле Ван Гога - экспрессионизм, яркие мазки"""

    def __init__(self):
        super().__init__('vangogh', 'экспрессионист')
    ...
    def generate_proposals_from_prompt(self, prompt: str) -> List[str]:
        ...
        if 'звёзд' in prompt_lower or 'ночь' in prompt_lower:
            proposals = ['Звёздная ночь']
        elif 'цвет' in prompt_lower or 'поле' in prompt_lower:
            proposals = ['Подсолнухи', 'Ирисы', 'Пшеничное поле с кипарисами']
        elif 'эмоци' in prompt_lower or 'экспресс' in prompt_lower:
            proposals = ['Звёздная ночь', 'Автопортрет']
        else:
    ...
    def generate_argument(self, proposal):
        args = {
            'Звёздная ночь': 'Вихри эмоций на холсте!',
            'Подсолнухи': 'Солнечная энергия жизни!',
            'Ирисы': 'Красота в каждом мазке!',
            'Автопортрет': 'Душа художника обнажена!',
            'Пшеничное поле с кипарисами': 'Величие природы в движении!'
        }
        return args.get(proposal, 'Каждый мазок — эмоция!')
