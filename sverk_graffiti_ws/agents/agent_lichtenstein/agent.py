...
# Специфичный агент Лихтенштейна

class LichtensteinAgent(ArtAgent):
    """Агент в стиле Лихтенштейна - поп-арт, комиксы, точки Бен-Дей"""

    def __init__(self):
        super().__init__('lichtenstein', 'поп-арт художник')
    ...
    def generate_proposals_from_prompt(self, prompt: str) -> List[str]:
        ...
        if 'комикс' in prompt_lower or 'поп-арт' in prompt_lower:
            proposals = ['WHAAM!', 'Комикс-панель']
        elif 'ярк' in prompt_lower or 'цвет' in prompt_lower:
            proposals = ['Поп-арт взрыв', 'Девушка с лентой']
        elif 'точк' in prompt_lower or 'бен-дей' in prompt_lower:
            proposals = ['Точки Бен-Дей']
        else:
    ...
    def generate_argument(self, proposal):
        args = {
            'WHAAM!': 'Энергия комиксов!',
            'Девушка с лентой': 'Икона поп-культуры!',
            'Поп-арт взрыв': 'Яркость массового искусства!',
            'Комикс-панель': 'История в одном кадре!',
            'Точки Бен-Дей': 'Техника печати как искусство!'
        }
        return args.get(proposal, 'Искусство для всех!')


if __name__ == '__main__':
    agent = LichtensteinAgent()
    agent.run()